...
if load_data: # ENSURE THAT THE FILES ARE SAVED ALREADY
    evaluated_set_X_batch = pd.read_csv("_islands_budget_" + str(budget) + "_X.csv", index_col = 0).values
    evaluated_set_y_batch = pd.read_csv("_islands_budget_" + str(budget) + "_y.csv", index_col = 0).values
    oos_set = pd.read_csv("_islands_budget_" + str(budget) + "_X_oos.csv", index_col = 0).values
    y_test = pd.read_csv("_islands_budget_" + str(budget) + "_y_oos.csv",index_col=0).values
else:
    ...
    pd.DataFrame(evaluated_set_X_batch).to_csv("_islands_budget_" + str(budget) + "_X.csv")
    pd.DataFrame(evaluated_set_y_batch).to_csv("_islands_budget_" + str(budget) + "_y.csv")
    ...
    pd.DataFrame(oos_set).to_csv("_islands_budget_" + str(budget) + "_X_oos.csv")
    pd.DataFrame(y_test).to_csv("_islands_budget_" + str(budget) + "_y_oos.csv")
...
pd.DataFrame(y_hat_test[0]).to_csv("_islands_budget_" + str(budget) + "_y_hat_GaussianProcessRegressor.csv")
pd.DataFrame(y_hat_test[1]).to_csv("_islands_budget_" + str(budget) + "_y_hat_GradientBoostingRegressor.csv")
...
fig.savefig("_islands_GradientBoostingRegressor_GaussianProcessRegressor" + str(budget) + ".png");